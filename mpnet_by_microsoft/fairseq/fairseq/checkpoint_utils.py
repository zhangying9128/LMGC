...
def save_checkpoint(args, trainer, epoch_itr, val_loss):
    ...
    '''
    checkpoint_conds['checkpoint_{}_{}.pt'.format(epoch, updates)] = (
        not end_of_epoch and not args.no_epoch_checkpoints and args.save_interval_updates > 0 and
        updates % args.save_interval_updates == 0
    )
...
def _upgrade_state_dict(state):
    ...
    #zhangying update task name
    if state['args'].task == 'mixed_position_lm':
        state['args'].task = 'masked_permutation_lm'
        state['args'].criterion = 'masked_permutation_cross_entropy'
