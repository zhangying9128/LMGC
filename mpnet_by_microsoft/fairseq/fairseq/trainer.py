...
class Trainer(object):
    ...
    def get_decayed_param_groups(self, params, num_layers, lr, lr_decay=1.0, weight_decay=0.0):
        import re
        decayed_params = []
        for k, v in params:
            ...
            if 'sentence_encoder.layers' in k:
                ...
                layer = num_layers
            factor = factor * lr_decay ** (num_layers - layer)

            param = {
                'params': v,
                'lr':     lr * factor,
                'weight_decay': 0.0 if 'layer_norm' in k or 'bias' in k else weight_decay
            }
            decayed_params.append(param)
        return decayed_params

    def _build_optimizer(self):
        if self.args.do_layer_decay:
            params = chain(self.model.named_parameters(), self.criterion.named_parameters())
            params = self.get_decayed_param_groups(
                params, 
            ...
            assert not self.args.fp16 or (self.args.fp16 and self.args.memory_efficient_fp16)
        else:
            params = list(
                filter(
    ...
    def load_checkpoint(
        ...
        if os.path.exists(filename):
            ...
            for key in self.model.state_dict().keys():
                if key not in state['model'].keys():
                    print(2, key)
            '''
            try:
